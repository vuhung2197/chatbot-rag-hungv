...
export async function googleCallback(req, res) {
    ...
    try {
        ...
        // Login mode: Upsert User to handle Find-or-Create atomically
        // Using empty string '' for password_hash to satisfy potential NOT NULL constraints
        ...
        console.log('✅ User login/register successful:', { id: user.id, email: user.email });

        // Ensure wallet exists for every user
        const [wallets] = await pool.execute('SELECT id FROM user_wallets WHERE user_id = ?', [user.id]);
        if (wallets.length === 0) {
            try {
                await pool.execute(
