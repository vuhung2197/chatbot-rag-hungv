...
function redirectWithError(res, errorCode) {
    return res.redirect(`${getFrontendUrl()}?error=${errorCode}`);
}

// ─── Helper: Get Google user profile from OAuth code ───
...
// ─── Helper: Handle OAuth link mode ───
async function handleLinkMode(res, { linkUserId, googleId, email, tokens }) {
    const userId = parseInt(linkUserId);
    const user = await authService.findUserById(userId);
    if (!user) return redirectWithError(res, 'user_not_found');

    const existingLink = await authService.findOAuthLink('google', googleId);
    if (existingLink && existingLink.user_id !== userId) {
        return redirectWithError(res, 'already_linked_to_another_account');
    }

    await authService.linkOAuthProvider({
        userId, provider: 'google', providerUserId: googleId, email, tokens
    });

    return res.redirect(`${getFrontendUrl()}/profile?oauth_linked=google&success=true`);
}

// ─── Helper: Handle OAuth login mode ───
async function handleLoginMode(req, res, { name, email, googleId, picture, tokens }) {
    ...
    if (!isLinked) {
        await authService.linkOAuthProvider({
            userId: user.id, provider: 'google', providerUserId: googleId, email, tokens
        });
    }

    const isNewUser = !(user.password_hash && user.password_hash.trim() !== '');
    ...
    if (isNewUser) {
        return res.redirect(`${frontendUrl}/set-password${separator}token=${jwtToken}&role=${user.role}&id=${user.id}&newUser=true`);
    }

    res.redirect(`${frontendUrl}${separator}token=${jwtToken}&role=${user.role}&id=${user.id}`);
}
...
export async function googleCallback(req, res) {
    ...
    if (!verifyStateCookie(req, state)) {
        return redirectWithError(res, 'csrf');
    }
    res.clearCookie('oauth_state');

    try {
        const profile = await getGoogleProfile(code);
        if (!profile.email) return redirectWithError(res, 'no_email');

        if (oauthMode === 'link' && linkUserId) {
            return await handleLinkMode(res, { linkUserId, ...profile });
        }

        await handleLoginMode(req, res, profile);

    } catch (error) {
        console.error('❌ Error in Google OAuth callback:', error);
        res.redirect(`${getFrontendUrl()}?error=oauth_failed&message=${encodeURIComponent(error.message)}`);
    }
}
...
export async function linkOAuthProvider(req, res) {
    try {
        ...
        if (provider === 'google') {
            ...
            return res.json({ message: 'Redirecting to OAuth provider...', redirectUrl: url });
        }
...
export async function logout(req, res) {
    try {
        ...
        res.json({ message: 'Logged out successfully', sessionDeleted: true });
    } catch (error) {
