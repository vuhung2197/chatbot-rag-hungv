...
- CHỈ TRẢ VỀ CÂU HỎI ĐÃ VIẾT LẠI. KHÔNG trả lời câu hỏi.`;
    ...
    async processChat({ userId, message, model, conversationId }) {
        ...
        console.log(`🧭 Intent: ${intent} | ${reasoning} | userId: ${userId}`);
        console.log(`🔍 Debug - intent type: ${typeof intent}, value: "${intent}", userId: ${userId}`);

        // Check if user asking about their progress/stats
        if (intent === 'USER_PROGRESS') {
            if (userId) {
                console.log('📊 Routing to MCP USER_PROGRESS handler');
                return await this._handleProgressQuery({ userId, message, processingMessage, history, modelConfig, conversationId });
            } else {
                // No userId, return friendly message
                return {
                    reply: 'Bạn cần đăng nhập để xem tiến độ học tập của mình.',
                    reasoning_steps: [`Intent: USER_PROGRESS`, 'Action: Require authentication'],
                    chunks_used: []
                };
            }
        }
    ...
    async streamChat({ userId, message, model, conversationId }, sendEvent) {
        ...
        try {
            ...
            if (intent === INTENTS.USER_PROGRESS) {
                if (userId) {
                    streamResult = await this._streamUserProgress({ userId, message, processingMessage, history, modelConfig, sendEvent });
                } else {
                    streamResult = { reply: 'Bạn cần đăng nhập để xem tiến độ học tập của mình.', chunksUsed: [], reasoningDetail: ['Require authentication'] };
    ...
    async _streamUserProgress({ userId, message, processingMessage, history, modelConfig, sendEvent }) {
        sendEvent('status', { content: '📊 Đang lấy thông tin tiến độ học tập...' });
        try {
            // Basic progress
            const [progressRows] = await pool.execute(
                `SELECT COUNT(DISTINCT uv.id) as vocabulary_count, COUNT(DISTINCT CASE WHEN uv.mastery >= 3 THEN uv.id END) as mastered_words,
                COUNT(DISTINCT ls.id) as listening_completed, COUNT(DISTINCT rs.id) as reading_completed,
                COUNT(DISTINCT ss.id) as speaking_completed, COUNT(DISTINCT ws.id) as writing_completed
                FROM users u LEFT JOIN user_vocabulary uv ON u.id = uv.user_id
                LEFT JOIN listening_submissions ls ON u.id = ls.user_id LEFT JOIN reading_submissions rs ON u.id = rs.user_id
                LEFT JOIN speaking_submissions ss ON u.id = ss.user_id LEFT JOIN writing_submissions ws ON u.id = ws.user_id
                WHERE u.id = ?`, [userId]
            );

            // Vocabulary details
            const [vocabByLevel] = await pool.execute(
                `SELECT level, COUNT(*) as count FROM user_vocabulary WHERE user_id = ? GROUP BY level ORDER BY level`, [userId]
            );
            const [vocabByTopic] = await pool.execute(
                `SELECT topic, COUNT(*) as count FROM user_vocabulary WHERE user_id = ? AND topic IS NOT NULL GROUP BY topic ORDER BY count DESC LIMIT 5`, [userId]
            );
            const [recentVocab] = await pool.execute(
                `SELECT word, level, mastery FROM user_vocabulary WHERE user_id = ? ORDER BY updated_at DESC LIMIT 5`, [userId]
            );

            // Listening details
            const [listeningStats] = await pool.execute(
                `SELECT AVG(score_total) as avg_score, COUNT(*) as total FROM listening_submissions WHERE user_id = ?`, [userId]
            );

            // Reading details
            const [readingStats] = await pool.execute(
                `SELECT AVG(score_total) as avg_score, COUNT(*) as total FROM reading_submissions WHERE user_id = ?`, [userId]
            );

            // Speaking details
            const [speakingStats] = await pool.execute(
                `SELECT AVG(score_total) as avg_score, COUNT(*) as total FROM speaking_submissions WHERE user_id = ?`, [userId]
            );

            // Writing details
            const [writingStats] = await pool.execute(
...
- Huy hiệu: ${Array.isArray(learningStreaks[0].badges) ? learningStreaks[0].badges.join(', ') : (typeof learningStreaks[0].badges === 'string' ? JSON.parse(learningStreaks[0].badges).join(', ') : 'Chưa có')}` : '- Chưa có dữ liệu'}`;
            ...
            sendEvent('text', { content: reply });
            return { reply, chunksUsed: [], sourceType: 'db_direct', reasoningDetail: ['Data source: Direct Database Query'] };
        } catch (error) {
            console.error('❌ Stream Progress Error:', error);
            const errorMsg = 'Xin lỗi, không thể lấy thông tin tiến độ.';
            sendEvent('text', { content: errorMsg });
            return { reply: errorMsg, chunksUsed: [], reasoningDetail: ['DB Error'] };
        }
    }

    // ─── Handler: User Progress Query ───
    async _handleProgressQuery({ userId, message, processingMessage, history, modelConfig, conversationId }) {
        const t0 = Date.now();
...
- Huy hiệu: ${Array.isArray(learningStreaks[0].badges) ? learningStreaks[0].badges.join(', ') : (typeof learningStreaks[0].badges === 'string' ? JSON.parse(learningStreaks[0].badges).join(', ') : 'Chưa có')}` : '- Chưa có dữ liệu'}`;
            ...
            if (conversationId) {
                const finalConversationId = await saveChatAndTrack(this, {
                    userId, conversationId, message, reply,
                    metadata: { processing_time: processTime, model: modelConfig.name, source: 'db_direct' },
                    usageType: 'progress_query', usageData: { tokens: context.length }
                });
                return { reply, conversationId: finalConversationId, chunks_used: [], reasoning_steps: reasoningSteps, source_type: 'db_direct' };
            }
            return { reply, chunks_used: [], reasoning_steps: reasoningSteps, source_type: 'db_direct' };
        } catch (error) {
            console.error('❌ Progress Query Error:', error);
            console.error('Error stack:', error.stack);
            return { reply: `Xin lỗi, không thể lấy thông tin tiến độ học tập. Lỗi: ${error.message}`, chunks_used: [], reasoning_steps: ['DB Error: ' + error.message] };
        }
    }
}
