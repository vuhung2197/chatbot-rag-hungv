...
// Helper to get upload dir relative to project root
// Since this file is in backend/src/modules/user, 
// and root is backend/, we need to go up 3 levels
// But to be consistent with uploads structure: backend/uploads/avatars
const getUploadsDir = () => path.join(process.cwd(), 'uploads');
const getAvatarsDir = () => path.join(getUploadsDir(), 'avatars');
...
export async function uploadAvatar(req, res) {
    try {
        ...
        if (oldAvatarUrl && oldAvatarUrl.startsWith('/uploads/avatars/')) {
            const oldPath = path.join(process.cwd(), oldAvatarUrl); // use absolute path
