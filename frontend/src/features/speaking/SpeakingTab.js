...
// ─── Sub-component: Speaking Selector ───
function SpeakingSelector({ type, setType, level, setLevel, darkMode }) {
    return (
...
// ─── Sub-component: Speaking Topic List ───
function SpeakingTopicList({ topics, type, level, isLoading, isGenerating, handleGenerateTopic, setCurrentView, setSelectedTopic, darkMode }) {
    return (
        <>
...
export default function SpeakingTab({ darkMode }) {
    ...
    const renderDashboard = () => (
        <div style={{ maxWidth: type === 'pronunciation' ? '1200px' : '800px', margin: '0 auto', ...themeVars }}>
            ...
            {/* Selector Form */}
            <SpeakingSelector type={type} setType={setType} level={level} setLevel={setLevel} darkMode={darkMode} />

            {/* List or Game Start */}
            {type !== 'pronunciation' ? (
                <SpeakingTopicList
                    topics={topics} type={type} level={level} isLoading={isLoading}
                    isGenerating={isGenerating} handleGenerateTopic={handleGenerateTopic}
                    setCurrentView={setCurrentView} setSelectedTopic={setSelectedTopic} darkMode={darkMode}
                />
            ) : (
                <SpeakingIpaChart
                    darkMode={darkMode}
